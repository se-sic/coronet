...
patrick::with_parameters_test_that("Split a data object time-based (split.basis = 'mails').", {
    ...
    expected.data = list(
        ...
        issues = list(
            ...
            "2010-10-10 06:38:13-2013-10-10 00:38:13" = data$issues[rownames(data$issues) %in% 1:15, ],
...
patrick::with_parameters_test_that("Split a data object time-based (split.basis = 'issues').", {
    ...
    expected.data = list(
        ...
        issues = list(
            "2013-04-21 23:52:09-2015-04-22 11:52:09" = data$issues[rownames(data$issues) %in% 1:15, ],
            "2015-04-22 11:52:09-2017-04-21 23:52:09" = data$issues[rownames(data$issues) %in% c(16:38, 41:55), ],
            "2017-04-21 23:52:09-2017-05-23 12:32:40" = data$issues[rownames(data$issues) %in% 39:40, ]
        ),
        mails = list(
...
patrick::with_parameters_test_that("Split a data object time-based (split.basis = 'mails', sliding.window = TRUE).", {
    ...
    expected.data = list(
        ...
        issues = list(
            ...
            "2010-10-10 06:38:13-2013-10-10 00:38:13" = data$issues[rownames(data$issues) %in% 1:15, ],
            "2012-04-10 03:38:13-2015-04-10 21:38:13" = data$issues[rownames(data$issues) %in% 1:15, ],
            "2013-10-10 00:38:13-2016-07-12 16:05:38" = data$issues[rownames(data$issues) %in% c(16:17, 22:24, 31:33, 42:45, 48:51, 53:55), ]
        ),
        mails = list(
            ## comments indicate row names when pasta is not configured
...
patrick::with_parameters_test_that("Split a data object time-based (split.basis = 'issues', sliding.window = TRUE).", {
    ...
    expected.data = list(
        ...
        issues = list(
            ...
            "2015-04-22 11:52:09-2017-04-21 23:52:09" = data$issues[rownames(data$issues) %in% c(16:38, 41:55), ],
            "2016-04-21 17:52:09-2017-05-23 12:32:40" = data$issues[rownames(data$issues) %in% c(16:55), ]
        ),
        mails = list(
            ## comments indicate row names when pasta is not configured
            "2013-04-21 23:52:09-2015-04-22 11:52:09" = data$mails[0, ],
...
patrick::with_parameters_test_that("Split a data object time-based (bins = ... , sliding.window = TRUE).", {
    ...
    expected.data = list(
        ...
        issues = list(
            "2016-01-01 00:00:00-2016-12-31 23:59:59" = data$issues[rownames(data$issues) %in% c(16:38, 41:55), ],
            "2016-12-31 23:59:59-2017-06-03 03:03:03" = data$issues[rownames(data$issues) %in% 39:40, ]
        ),
        mails = list(
            "2016-01-01 00:00:00-2016-12-31 23:59:59" = data$mails[rownames(data$mails) %in% 13:17, ],
...
patrick::with_parameters_test_that("Split a data object time-based with equal-sized windows (number.windows = ..., split.basis = 'mails').", {
    ...
    expected.data = list(
        ...
        issues = list(
            ...
            "2010-08-26 17:21:55-2013-08-04 04:43:46" = data$issues[rownames(data$issues) %in% 1:15, ],
...
patrick::with_parameters_test_that("Split a data object time-based with equal-sized windows (number.windows = ..., split.basis = 'issues').", {
    ...
    expected.data = list(
        ...
        issues = list(
            "2013-04-21 23:52:09-2014-09-01 12:05:39" = data$issues[rownames(data$issues) %in% 1:15, ],
            "2014-09-01 12:05:39-2016-01-12 00:19:09" = data$issues[0, ],
            "2016-01-12 00:19:09-2017-05-23 12:32:40" = data$issues[rownames(data$issues) %in% 16:55, ]
        ),
        mails = list(
