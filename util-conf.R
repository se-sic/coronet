...
CodefaceConf = R6::R6Class(
  ...
  private = list(
    ...
    get.callgraph.folder = function(data, selection.process, casestudy) {
      return(file.path(
        ...
        private$subfolder.results,
        selection.process,
        paste(casestudy, "callgraphs", sep = "_")
      ))
    },
    ...
    get.synchronicity.folder = function(data, selection.process, casestudy) {
      return(file.path(
        ...
        private$subfolder.results,
        selection.process,
        paste(casestudy, "synchronicity", sep = "_")
      ))
    },
    ...
    construct.conf.path = function(data,
                                   selection.process,
                                   casestudy,
                                   tagging) {
      ...
      conf.file = file.path(private$get.configurations.folder(data, selection.process),
                            conf.basename)
    ...
    load.configuration = function(data,
                                  selection.process,
                                  casestudy,
                                  artifact = "feature") {
      ...
      if (is.null(tagging)) {
        logging::logerror(
          "Artifact '%s' cannot be converted to a proper Codeface tagging! Stopping...",
          artifact
        )
      ...
      if (!is.null(revisions.dates))
        names(revisions.dates) = revisions
  ...
  public = list(
    ...
    initialize = function(data,
      ...
      if (!missing(selection.process) &&
          is.character(selection.process)) {
